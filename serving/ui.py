...
# Import function from sample_retrieval.py
SAMPLE_RETRIEVAL_PATH = "/home/mlops/Repository/data-pipeline-mlops/scripts"
if SAMPLE_RETRIEVAL_PATH not in sys.path:
    sys.path.insert(0, SAMPLE_RETRIEVAL_PATH)

try:
    from sample_retrieval import get_customer_features
    logger.info("Đã import thành công get_customer_features từ sample_retrieval.py")
except ImportError as e:
    logger.error(f"Không thể import get_customer_features: {e}")
    get_customer_features = None
...
def search_customer_data(customer_id: str) -> str:
    ...
    if get_customer_features is None:
        error_msg = "Cannot import get_customer_features from sample_retrieval.py"
